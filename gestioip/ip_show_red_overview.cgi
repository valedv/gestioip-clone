...
if ( $first_ip_int_ping && $first_ip_int_ping !~ /^\d{6,12}$/ ) {
	$gip->print_init("gestioip","$$lang_vars{redes_message}","$$lang_vars{redes_message}","$vars_file","$client_id");
	$gip->print_error("$client_id","$$lang_vars{formato_malo_message} $first_ip_int_ping") ;
}
if ( $last_ip_int_ping && $last_ip_int_ping !~ /^\d{6,12}$/ ) {
	$gip->print_init("gestioip","$$lang_vars{redes_message}","$$lang_vars{redes_message}","$vars_file","$client_id");
...
my @ip=$gip->get_host("$client_id","$first_ip_int","$last_ip_int");
...
for (my $ip_int_count = $redint; $i <= $anz_ip; $ip_int_count++) {
	...
	if ( $i == 1 ) {
		...
		if ( $view eq "short" ) {
			...
			print "<td bgcolor=\"$bgcolor\" align=\"center\" valign=\"middle\"><form method=\"POST\" action=\"$server_proto://$base_uri/res/ip_modip_form.cgi\" style=\"display:inline\"><input name=\"ip\" type=\"hidden\" value=\"$ip_int_count\"><input name=\"red_num\" type=\"hidden\" value=\"$red_num\"><input name=\"loc\" type=\"hidden\" value=\"$loc\"><input type=\"hidden\" name=\"client_id\" value=\"$client_id\"><input type=\"hidden\" name=\"ip_version\" value=\"$ip_version\"><input type=\"submit\" value=\"$fourth_oct\" title=\"$ip_title\" name=\"B2\" class=\"$button_class\"></form></td>\n";
		}
		if ( $i == $j ) {
			print "</tr>\n<tr>";
			$j = $j + 25;
