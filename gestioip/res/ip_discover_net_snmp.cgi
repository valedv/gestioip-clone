...
my $red_loc_id = $gip->get_loc_id("$client_id","$red_loc");
...
for (my $i = $first_ip_int; $i <= $last_ip_int; $i++) {
	...
	my $exit=0;
	my $node=$gip->int_to_ip("$client_id","$i","$ip_version");
	my $node_id=$gip->get_host_id_from_ip("$client_id","$node") || "";
		...
		$pid = $pm->start("$node") and next;
			...
			if ( $utype_db eq "man" ) {
				print "update type: $utype_db - $$lang_vars{ignorado_message}<br>\n";
				$exit = 1;
...
#				print "SNMP $$lang_vars{can_not_connect_message}<br>\n";
			}

			if ( ( $snmp_info_connect == "1" && $snmp_connect == "1" ) ) {
				print "$$lang_vars{can_not_connect_message}<br>\n";
				$exit = "1";
				$pm->finish($exit); # Terminates the child process
			}
			...
			if ( ! $node_id && $device_name ) {
				...
				print "$$lang_vars{added_message}: $device_name";
				...
				$new_host = "1";
				$node_id=$gip->get_last_host_id("$client_id");
				print "$$lang_vars{added_message}: $device_name_dns";
				...
				if ( $device_name ) {
					...
					print "$$lang_vars{added_message}: unknown";
					$new_host = "1";
				}
			}
			...
			if ( $hostname_update == "1" && $new_host == "0" ) { 
				print "$$lang_vars{host_updated_message}: $device_name\n";
			}
			if ( $update == "1" && $new_host == "0" ) {
				...
				print "$$lang_vars{no_changes_message}\n";
