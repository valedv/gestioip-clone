...
print "<tr><td>$$lang_vars{all_sheets_message}</td><td> <input type=\"radio\" name=\"sheet_import_type\" value=\"all_sheet\" onclick=\"hoja.disabled=true;some_sheet_values.disabled=true\" checked></td></tr>\n";
...
print "<p>$$lang_vars{import_host_ip_format_message}<p>\n";

print "<table border=\"0\" cellpadding=\"5\">\n";
print "<tr><td><input type=\"radio\" name=\"IP_format\" id=\"IP_format1\" value=\"standard\" onclick=\"network_field.disabled=true;network_field.value = ''\" checked> $$lang_vars{import_host_standard_message}\n";
print "<tr><td><input type=\"radio\" name=\"IP_format\" id=\"IP_format2\" value=\"last_oct\" onclick=\"network_field.disabled=false;\"> $$lang_vars{import_hosts_only_last_oct_message} <input type=\"text\" name=\"network_field\" size=\"5\" maxlength=\"5\" disabled> $$lang_vars{network_address_field_message}</td></tr>\n";
print "</table>\n";
...
print "<tr><td><select name=\"ip\" size=\"1\">";
my @nums=("","A","B","C","D","E","F","G","H","I","J","K","L","M");
foreach (@nums) {
	print "<option>$_</option>\n";
}
print "</select>\n";
print "</td><td>$$lang_vars{ip_address_message}</td></tr>\n";
print "<tr><td><select name=\"hostname\" size=\"1\">";
foreach (@nums) {
	print "<option>$_</option>\n";
}
print "</select>\n";
print "</td><td>$$lang_vars{hostname_message}</td></tr>\n";

print "<tr><td colspan=\"2\"><br><i>$$lang_vars{columnas_opcionales_message}</i>\n";
...
print "<tr><td><br><p><br>$$lang_vars{force_update_type_message}</td>\n";
print "<td><br><select name=\"update_type\" size=\"1\">";
print "<option></option>";
my $j=0;
...
print "<tr><td colspan=\"2\"><br><input name=\"spreadsheet\" type=\"hidden\" value=\"$filename\"><input name=\"client_id\" type=\"hidden\" value=\"$client_id\"><input type=\"submit\" value=\"$$lang_vars{importar_message}\" name=\"B1\" class=\"input_link_w\"></td></tr>\n";
print "</form>\n";
print "</table>\n";
