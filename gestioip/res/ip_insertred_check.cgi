...
### multiple_different_bm
	...
	foreach (@bitmasks) {
		...
		if ( $rootnet eq "n" ) {
			...
			my $red_check=$gip->check_red_exists("$client_id","$overlap_check_red","$BM");
			if ( $red_check ) {
				if ( $message ) {
					print "$message<font color=\"red\">$redob - NOT OK</font></b><p>$redob $$lang_vars{red_exists_message}<p><br><p>";
					print "<FORM><INPUT TYPE=\"BUTTON\" VALUE=\"$$lang_vars{atras_message}\" ONCLICK=\"history.go(-1)\" class=\"error_back_link\"></FORM>\n";
				}
				$gip->print_end("$client_id","$vars_file");
				exit 1;
