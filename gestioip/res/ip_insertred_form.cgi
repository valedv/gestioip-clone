...
$gip->print_init("$$lang_vars{crear_red_message}","$$lang_vars{crear_red_message}","$$lang_vars{crear_red_message}","$vars_file","$client_id","$ip_version");
...
$gip->print_error("$client_id","$$lang_vars{formato_malo_message}") if $ip_version !~ /(v4|v6)/ && $red_dat;
...
function checkRefresh(version) {
 if (version == 'v4') {
  ...
  if ( bm_index == "8" ) {
     ...
     document.forms.insertred_form.BM.options[1].selected=true
  ...
  if ( bm_index == "64" ) {
     ...
     document.forms.insertred_form.BM.options[119].selected=true
     document.forms.insertred_same_bm_form.BM.options[119].selected=true
  }
 }
}
-->
   ...
   var values_v4=new Array("CLASS A","255.0.0.0 - 16.777.214 hosts","255.128.0.0 - 8.388.606 hosts","255.192.0.0 - 4.194.302 hosts","255.224.0.0 - 2.097.150 hosts","255.240.0.0 - 1.048.574 hosts","255.248.0.0 - 524.286 hosts","255.252.0.0 - 262.142 hosts","255.254.0.0 - 131.070 hosts","CLASS B","255.255.0.0 - 65.534 hosts","255.255.128.0 - 32766 hosts","255.255.192.0 - 16.382 hosts","255.255.224.0 - 8.190 hosts","255.255.240.0 - 4.094 hosts","255.255.248.0 - 2.046 hosts","255.255.252.0 - 1.022 hosts","255.255.254.0 - 510 hosts","CLASS C","255.255.255.0 - 254 hosts","255.255.255.128 - 126 hosts","255.255.255.192 - 62 hosts","255.255.255.224 - 30 hosts","255.255.255.240 - 14 hosts","255.255.255.248 - 6 hosts","255.255.255.252 - 2 hosts","255.255.255.254 - 0 hosts","255.255.255.255 - 0 hosts")
	...
	document.getElementById('example_network_message_different_bm').innerHTML = '<i>$$lang_vars{example_network_message}</i>';
       ...
       for(i=0;i<128;i++){
          ...
          document.insertred_form.BM.options[119].selected = true
          document.insertred_form.BM.options[i].disabled=false
          document.insertred_same_bm_form.BM.options[i].value=j
          document.insertred_same_bm_form.BM.options[i].text=values_v6[i]
          document.insertred_same_bm_form.BM.options[119].selected = true
          document.insertred_same_bm_form.BM.options[i].disabled=false
          j++
       }
	document.getElementById('example_network_message').innerHTML = '<i>$$lang_vars{example_network_v6_message}</i>';
...
if ( $ipv4_only_mode eq "no" ) {
	if ( $red_dat ) {
		if ( $ip_version eq "v4" ) {
print <<EOF;
			<tr><td></td><td>IPv4<input type="radio" name="ip_version" value="v4" onchange="change_BM_select('v4','$$lang_vars{entradas_redes_message}');" checked> IPv6<input type="radio" name="ip_version" value="v6" onchange="change_BM_select('v6',$$lang_vars{entradas_redes_message});" disabled></td></tr>\n
EOF
		} else {
print <<EOF;
			<tr><td></td><td>IPv4<input type="radio" name="ip_version" value="v4" onchange="change_BM_select('v4','$$lang_vars{entradas_redes_message}');" disabled> IPv6<input type="radio" name="ip_version" value="v6" onchange="change_BM_select('v6',$$lang_vars{entradas_redes_message});" checked></td></tr>\n
EOF
			}
	} else {
		if ( $ip_version eq "v4" ) {
print <<EOF;
			<tr><td></td><td>IPv4<input type="radio" name="ip_version" value="v4" onchange="change_BM_select('v4','$$lang_vars{entradas_redes_message}');" checked> IPv6<input type="radio" name="ip_version" value="v6" onchange="change_BM_select('v6','$$lang_vars{entradas_redes_message}');"></td></tr>\n
EOF
		} else {
print <<EOF;
			<tr><td></td><td>IPv4<input type="radio" name="ip_version" value="v4" onchange="change_BM_select('v4','$$lang_vars{entradas_redes_message}');"> IPv6<input type="radio" name="ip_version" value="v6" onchange="change_BM_select('v6','$$lang_vars{entradas_redes_message}');" checked></td></tr>\n
EOF
		}
	}
}
...
if ( $red_dat ) {
	if ( $ip_version eq "v4" ) {
		print "<tr><td align=\"right\">$$lang_vars{redes_message}</td><td><input name=\"red\" id=\"red\" type=\"text\" size=\"15\" maxlength=\"40\" value=\"$ip_freerange\"> <span id=\"example_network_message\"><i>$$lang_vars{example_network_message}</i></span></td></tr>\n";
	} else {
		print "<tr><td align=\"right\">$$lang_vars{redes_message}</td><td><input name=\"red\" id=\"red\" type=\"text\" size=\"40\" maxlength=\"40\" value=\"$ip_freerange\"> <span id=\"example_network_message\"><i>$$lang_vars{example_network_v6_message}</i></span></td></tr>\n";
	}
} else {
	if ( $ip_version eq "v4" ) {
		print "<tr><td align=\"right\">$$lang_vars{redes_message}</td><td><input name=\"red\" id=\"red\" type=\"text\" size=\"15\" maxlength=\"40\"> <span id=\"example_network_message\"> <span id=\"example_network_message\"><i>$$lang_vars{example_network_message}</i></span></td></tr>\n";
	} else {
		print "<tr><td align=\"right\">$$lang_vars{redes_message}</td><td><input name=\"red\" id=\"red\" type=\"text\" size=\"40\" maxlength=\"40\"> <span id=\"example_network_message\"> <span id=\"example_network_message\"><i>$$lang_vars{example_network_v6_message}</i></span></td></tr>\n";
	}
}
...
if ( $ip_version eq "v4" ) {
	...
	for (my $i = 1; $i <= 128; $i++) {
		...
		if ( ! $red_dat ) {
			if ( $i eq "120") {
				print "<option value=\"$i\" selected><b>$i</b> ($anz_hosts_bm{$i} $host_red_noti)</option>";
			} else {
				print "<option value=\"$i\">$i ($anz_hosts_bm{$i} $host_red_noti)</option>";
			}
		} else {
			next if $i < $BM_freerange;
			if ( $i == "120" ) {
				print "<option selected><b>$i</b> ($anz_hosts_bm{$i} $host_red_noti)</option>";
			} elsif ( $i eq "$BM_freerange" && $BM_freerange > "120") {
				print "<option selected><b>$i</b> ($anz_hosts_bm{$i} $host_red_noti)</option>";
			} else {
				print "<option>$i ($anz_hosts_bm{$i} $host_red_noti)</option>";
			}
		}
	}
}
...
print "<tr><td align=\"right\">$$lang_vars{rootnet_message}</td><td><input type=\"checkbox\" name=\"rootnet\" value=\"y\"><br></td></tr>\n ";
...
if ( $red_dat ) {
	if ( $ip_version eq "v4" ) {
		print "<tr><td align=\"right\">$$lang_vars{redes_primero_message}</td><td><input name=\"red\" id=\"red\" type=\"text\" size=\"15\" maxlength=\"40\" value=\"$ip_freerange\">  <span id=\"example_network_message_same_bm\"><i>$$lang_vars{example_network_message}</i></span></td></tr>\n";
	} else {
		print "<tr><td align=\"right\">$$lang_vars{redes_primero_message}</td><td><input name=\"red\" id=\"red\" type=\"text\" size=\"40\" maxlength=\"40\" value=\"$ip_freerange\"> <span id=\"example_network_message_same_bm\"><i>$$lang_vars{example_network_v6_message}</i></span></td></tr>\n";
	}
} else {
	if ( $ip_version eq "v4" ) {
		print "<tr><td align=\"right\">$$lang_vars{redes_message}</td><td><input name=\"red\" id=\"red\" type=\"text\" size=\"15\" maxlength=\"40\"> <span id=\"example_network_message_same_bm\"> <span id=\"example_network_message_same_bm\"><i>$$lang_vars{example_network_message}</i></span></td></tr>\n";
	} else {
		print "<tr><td align=\"right\">$$lang_vars{redes_message}</td><td><input name=\"red\" id=\"red\" type=\"text\" size=\"40\" maxlength=\"40\"> <span id=\"example_network_message_same_bm\"> <span id=\"example_network_message_same_bm\"><i>$$lang_vars{example_network_v6_message}</i></span></td></tr>\n";
	}
}
	...
	for (my $i = 1; $i <= 128; $i++) {
		if ( ! $red_dat ) {
			...
			if ( $max_redes < "50" ) {
				...
				if ( $i == "120" ) {
					if ( $max_redes == "1" ) {
						print "<option value=\"$i\" selected><b>$i</b> - $max_redes $$lang_vars{max_red_message}</option>";
					} else {
						print "<option value=\"$i\" selected><b>$i</b> - $max_redes $$lang_vars{max_redes_message}</option>";
					}
				} elsif ( $i eq "$BM_freerange" && $BM_freerange > "120") {
					if ( $max_redes == "1" ) {
						print "<option value=\"$i\" selected><b>$i</b> - $max_redes $$lang_vars{max_red_message}</option>";
					...
					if ( $max_redes == "1" ) {
						print "<option value=\"$i\"><b>$i</b> - $max_redes $$lang_vars{max_red_message}</option>";
					} else {
						print "<option value=\"$i\">$i - $max_redes $$lang_vars{max_redes_message}</option>";
					}
				}
			} else {
				if ( $i == "120" ) {
...
print "<tr><td align=\"right\">$$lang_vars{rootnets_message}</td><td><input type=\"checkbox\" name=\"rootnet\" value=\"y\"><br></td></tr>\n ";
...
print <<EOF;
<td valign="middle"><span class="mostrar_link" onClick="calculate_same_BM('$ip_version')" style="cursor:pointer;" title="$$lang_vars{what_would_happen_message}"><br>&nbsp;&nbsp;$$lang_vars{calcular_message}<gspan></td></tr>\n
EOF
print "</table></form>\n";
...
if ( $red_dat ) {
	if ( $ip_version eq "v4" ) {
		print "<tr><td align=\"right\">$$lang_vars{redes_primero_message}</td><td><input name=\"red\" id=\"red\" type=\"text\" size=\"15\" maxlength=\"40\" value=\"$ip_freerange\">  <span id=\"example_network_message_different_bm\"><i>$$lang_vars{example_network_message}</i></span></td></tr>\n";
	} else {
		print "<tr><td align=\"right\">$$lang_vars{redes_primero_message}</td><td><input name=\"red\" id=\"red\" type=\"text\" size=\"40\" maxlength=\"40\" value=\"$ip_freerange\"> <span id=\"example_network_message_different_bm\"><i>$$lang_vars{example_network_v6_message}</i></span></td></tr>\n";
	}
} else {


	if ( $ip_version eq "v4" ) {
		print "<tr><td align=\"right\">$$lang_vars{redes_message}</td><td><input name=\"red\" id=\"red\" type=\"text\" size=\"15\" maxlength=\"40\"> <span id=\"example_network_message_different_bm\"> <span id=\"example_network_message_same_bm\"><i>$$lang_vars{example_network_message}</i></span></td></tr>\n";
	} else {
		print "<tr><td align=\"right\">$$lang_vars{redes_message}</td><td><input name=\"red\" id=\"red\" type=\"text\" size=\"40\" maxlength=\"40\"> <span id=\"example_network_message_different_bm\"> <span id=\"example_network_message_same_bm\"><i>$$lang_vars{example_network_v6_message}</i></span></td></tr>\n";
	}
}


print "<tr><td align=\"right\">$$lang_vars{bitmasks_message}</td><td><input name=\"bitmasks\" type=\"text\" size=\"15\" maxlength=\"200\"> <span id=\"bitmasks_format_message\"> $$lang_vars{bitmasks_format_message}</span></td></tr>\n";
print "<tr><td align=\"right\">$$lang_vars{loc_message}</td><td><select name=\"loc\" size=\"1\">";
print "<option></option>";
$j=0;
foreach (@values_locations) {
