...
if ( $which_clients !~ /^\d{1,4}/ ) {
	$gip->CheckInput("$client_id",\%daten,"$$lang_vars{mal_signo_error_message}","$$lang_vars{manage_custom_columns_message}: $$lang_vars{manage_manage_message} ","$vars_file");
...
if ( $management_type eq "insert_cc" ) {
	...
	foreach ( @ce_values ) {
		if ( "$custom_column" eq "$ce_values[$j]->[1]" ) {
			$gip->print_error("$client_id","$$lang_vars{ce_name_exists_message}");
		}
		$j++;
	}
	@cc_values=$gip->get_custom_columns("$client_id");
	...
	foreach ( @cc_values ) {
		if ( "$custom_column" eq "$cc_values[$j]->[0]" && ( "$cc_values[$j]->[2]" eq $client_id || "$cc_values[$j]->[2]" eq "9999" ) ) {
			$gip->print_error("$client_id","$$lang_vars{cc_name_exists_message}");
		}
		$j++;
	}
...
print "<table border=\"0\" cellpadding=\"25\" cellspacing=\"1\" width=\"100%\"><tr><td valign=\"top\" width=\"50%\">\n";
...
if ( ( $ce_values_count == "1" && $ce_values[0]->[1] eq "NOTYPE" )  ||  $ce_values_count == 0 ) {
	...
	if ( $anz_clients_all > 1 ) {
		print "<tr><td colspan=\"2\">$$lang_vars{all_clients_message}<input type=\"radio\" name=\"which_clients\" value=\"9999\" checked>&nbsp;&nbsp;&nbsp;$$lang_vars{actual_client_message}<input type=\"radio\" name=\"which_clients\" value=\"$client_id\"></td></tr>\n";
	}
...
if ( $anz_clients_all > 1 ) {
	print "<tr><td colspan=\"2\"><input name=\"client_id\" type=\"hidden\" value=\"$client_id\">$$lang_vars{all_clients_message}<input type=\"radio\" name=\"which_clients\" value=\"9999\" checked>&nbsp;&nbsp;&nbsp;$$lang_vars{actual_client_message}<input type=\"radio\" name=\"which_clients\" value=\"$client_id\"></td></tr>\n";
}
