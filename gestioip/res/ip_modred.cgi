...
if ( $BM ne $BM_new ) {
	...
	if ( $BM gt $BM_new ) {
		my @overlap_redes=$gip->get_overlap_red("$client_id");
		my @overlap_found = $gip->find_overlap_redes("$client_id","$red","$BM_new",\@overlap_redes,"$ip_version","$vars_file");
		if ( $overlap_found[1] ) {
			$gip->print_error("$client_id","$$lang_vars{rango_nuevo_message} $red/$BM_new $$lang_vars{overlaps_con_message} $overlap_found[1]"); 
		}
	} else {
		my $first_ip_int_del = "";
		my $last_ip_int_del = "";
		...
		$gip->delete_ip("$client_id","$first_ip_int_del","$last_ip_int_del");
	}
...
if ( $referer eq "host_list_view" && $BM_new != "32" ) {
	...
	my $anz_values_hosts=$anz_hosts_bm{$BM_new} || 254;
	...
	$gip->PrintIpTab("$client_id",$host_hash_ref,"$first_ip_int","$last_ip_int","res/ip_modip_form.cgi","$knownhosts","$$lang_vars{modificar_message}","$red_num","$loc","$vars_file","$anz_values_hosts","$start_entry_hosts","$entries_per_page_hosts","$host_order_by",$host_sort_helper_array_ref,"","$ip_version");
