...
my ($ip_hash,$host_sort_helper_array_ref)=$gip->get_host_hash("$client_id","$first_ip_old_int","$last_ip_old_int","IP_auf","hosts");
...
foreach my $ele (@new_redes_detail) {
	...
	my @rangos;
	@rangos = $gip->get_rangos("$client_id","$red_id_old");
	my $j = 0;
	foreach ( @rangos ) {
	...
	if ( $ip_version eq "v4" ) {
		...
		$SMALLEST_BM="$smallest_bm6" || "116";
	}
	if ( $keep_hosts eq "y" && $BM_nuevo >= $SMALLEST_BM ) {
			$gip->update_host_red_id_ip_all("$client_id","$red_id_nuevo","$first_ip_int","$last_ip_int");
			...
			if ( $ip_hash->{$last_ip_int}[12] && $ip_hash->{$last_ip_int}[12] =~ /\d/ ) {
				...
				$gip->delete_ip("$client_id","$last_ip_int","$last_ip_int");
			}
	} else {
