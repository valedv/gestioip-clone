...
print "<tr><td align=\"right\">$$lang_vars{BM_message}</td><td><select name=\"new_bm\" size=\"1\">\n";
if ( $ip_version eq "v4" ) {
	for (my $i = $smallest_valid_BM; $i < 31; $i++) {
		if ( $i =~ /^\d$/ ) {
...
print "<p><form name=\"splitred_form\" method=\"POST\" action=\"$server_proto://$base_uri/res/ip_splitred_check.cgi\">\n";
print "<table border=\"0\">\n";
print "<tr><td>$$lang_vars{bitmasks_message} <input name=\"bitmasks\" type=\"text\"  size=\"20\" maxlength=\"200\"></td></tr>\n";
print "</table>\n";
