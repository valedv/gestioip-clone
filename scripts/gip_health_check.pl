...
my $sid_gestioip="gestioip";
my $user_gestioip="gestioip";
my $pass_gestioip ="XXXXXXXX";
my $bbdd_host_gestioip="localhost";
...
if ( ! $location_check_null ) {
	print "*** table \"locations\" inconsistent ***\n";
	print "There is no row with the values \"-1\" and \"NULL\" - execute \"insert into locations (\"id\",\"loc\") values (\"-1\",\"NULL\");\"\n";
	$error="1";
}
if ( ! $categorias_check_null ) {
	print "*** table \"categorias\" inconsistent ***\n";
	print "There is no row with the values \"-1\" and \"NULL\" - execute \"insert into categorias (\"id\",\"cat\") values (\"-1\",\"NULL\");\"\n";
	$error="1";
}
...
sub check_douple_net {
	...
	my @values;
	my $dbh = mysql_verbindung($sid_gestioip,$user_gestioip,$pass_gestioip,$bbdd_host);
#	my $sth = $dbh->prepare("select INET_ATON(red), BM, descr, red from net where red in (select INET_ATON(red) from net group by red having count(1)>1) order by red;");
	my $sth = $dbh->prepare("select red, BM, descr from net WHERE client_id = \"$client_id\"  group by red having count(1)>1 order by red;");
