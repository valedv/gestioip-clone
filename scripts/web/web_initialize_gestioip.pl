...
my ( $disable_audit, $quiet, $help, $version_arg, $configuration_arg,$community,$snmp_version,$client_id,$lang,$ini_devices,$gip_config_file,$user,$add_comment, $with_spreadsheet,$max_sync_procs);
my $auth_proto="";
my $auth_pass="";
my $priv_proto="";
my $priv_pass="";
...
my $discover_snmp_net_args = " -c $community -s $snmp_version -i $client_id -d $ini_devices -g $gip_config_file -u $user $add_comment_arg -l $lang $auth_proto_arg $auth_pass_arg $priv_proto_arg $priv_pass_arg $sec_level_arg";

my $discover_snmp_host_args = " -s $snmp_version -i $client_id -g $gip_config_file -u $user $with_spreadsheet_arg -l $lang -p $max_sync_procs $auth_proto_arg $auth_pass_arg $priv_proto_arg $priv_pass_arg $sec_level_arg";

my $discover_dns_args = " -i $client_id -g $gip_config_file -u $user $with_spreadsheet_arg -l $lang -p $max_sync_procs";


unlink("$ini_stat") if -e $ini_stat;

print LOG "Creating ini_stat.html\n";
system("$dir/create_ini_stat.pl -g $gip_config_file -b 0 -a 0 -z 0 -l $lang");
