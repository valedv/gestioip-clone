...
#
#	my $err = $bridge->error();
#	print LOG "SNMP Community or Version probably wrong connecting to $node. $err\n" if defined $err;
...
print LOG "TEST: $client_id,$node,$community,$community_type,$snmp_version,$auth_pass,$auth_proto,$auth_is_key,$priv_proto,$priv_pass,$priv_is_key,$sec_level,$mibdirs_ref,$vars_file\n";
	...
	if ( $device_type_info && $device_vendor ) {
		if ( $node_name ) {
			print LOG "\n+++ Importing VLANs from $node ($node_name) ($device_vendor - $device_type_info) +++\n\n";
		} else {
			print LOG "\n+++ Importing VLANs from $node ($device_vendor - $device_type_info) +++\n\n";
		}
	} else {
		if ( $node_name ) {
			print LOG "\n+++ Importing VLANs from $node ($node_name) +++\n\n";
		} else {
			print LOG "\n+++ Importing VLANs from $node +++\n\n";
		}
	}
