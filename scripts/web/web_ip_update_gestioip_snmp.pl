...
my ( $disable_audit, $test, $mail, $help, $version_arg, $community_arg,$community,$client_id,$snmp_version,$lang,$gip_config_file,$user,$with_spreadsheet,$max_sync_procs);
my $auth_proto="";
my $auth_pass="";
my $priv_proto="";
my $priv_pass="";
...
#
				if ( ! defined($bridge) ) {
#	#				print LOG "SNMP::INFO: $lang_vars{can_not_connect_message}<br>\n";
#	#				$exit = 1;
#	#				$pm->finish($exit); # Terminates the child process
				} else {
...
#								);
				...
				if ( defined $session ) {
					...
					if ( ! ($session->{ErrorStr}) ) {
						...
						my @vendors=("actiontec","aficio|ricoh","alvaco""apple","aruba","adtran","allied","apc","altiga","arista","asante","avocent","axis","barracuda","billion","belair","bluecoat","borderware","brother","broadcom","brocade","calix","cyclades","canon","checkpoint","cisco","cyberoam","dell","dialogic","dothill","draytek","eci telecom","edgewater","emc","emerson","enterasys","epson","extreme","f5","fluke","fortinet","foundry","h3c","hp|hewlett.?packard","ibm","juniper","proxim","kodak","konica","lancom","alcatel|lucent","lexmark","linksys","lifesize","microsoft","minolta","motorola","netapp","netgear","nokia","nortel","novell","optibase","ovislink","panasonic","passport","palo.?alto","peplink","phaser","polycom","procurve","qnap","radvision","radware","realtek","riverstone","samsung","siemens","smc","sonicwall","stonesoft","sony","symantec","sun","tandberg","tippingpoint","toplayer","vegastream","vyatta","watchguard","websense","westbase","xerox","xiro","zyxel","3com");
