...
# setup_gestioip.sh VERSION 2.2.8.0
...
then
    ...
    echo "Linux distribution found: `cat /etc/issue`" >> $SETUP_LOG
fi
...
then
    ...
    if [ -z "$input" ] || [ "$input" = "y" ] || [ "$input" = "Y" ] || [ "$input" = "yes" ]
    then
	    echo "User choosed AUTOMATIC INSTALLATION OF PERL MODULES" >> $SETUP_LOG
		...
		then
			...
			then
				...
				rm -r ./netdisco-mibs-0.7* >> $SETUP_LOG 2>&1
				...
				$WGET -w 2 -T 5 -t 6 http://sourceforge.net/projects/netdisco/files/netdisco-mibs/0.7/netdisco-mibs-0.7.tar.gz >> $SETUP_LOG 2>&1
				if [ $? -ne 0 ]
				then
					...
					then
						...
						tar -vzxf netdisco-mibs-0.7.tar.gz >> $SETUP_LOG 2>&1
						mkdir -p /usr/share/gestioip/mibs  >> $SETUP_LOG 2>&1
						if [ -w "/usr/share/gestioip/mibs" ]
						then
							cp -r ./netdisco-mibs-0.7/* /usr/share/gestioip/mibs/ >> $SETUP_LOG 2>&1
				...
				echo
				echo "user choosed to install MIBs manually"  >> $SETUP_LOG
...
do
	...
	then
				...
				$WGET -w 2 -T 5 -t 6 http://search.cpan.org/~jmcnamara/ >> $SETUP_LOG 2>&1
			...
			then
				...
				$WGET -w 2 -T 5 -t 6 http://search.cpan.org/~dlux/ >> $SETUP_LOG 2>&1
			...
			then
				...
				$WGET -w 2 -T 5 -t 6 http://search.cpan.org/~chorny/ >> $SETUP_LOG 2>&1
			...
			then
				...
				$WGET -w 2 -T 5 -t 6 http://search.cpan.org/~maxb/ >> $SETUP_LOG 2>&1
			...
			then
				...
				$WGET -w 2 -T 5 -t 6 http://search.cpan.org/~olaf/ >> $SETUP_LOG 2>&1
			else
				continue
			fi
			...
			$WGET -w 2 -T 5 -t 6 $URL_COMPL >> $SETUP_LOG  2>&1
			if [ $? -ne 0 ]
			then
				echo " Failed" >> $SETUP_LOG
				echo " Failed"
			...
			then
				...
				then
					...
					sudo $MAKE install >> $SETUP_LOG 2>&1
					if [ $? -ne 0 ]
					then
						echo "FAILED"
...
#### XX
	...
	then
	   ...
	   echo "sudo apt-get install libdbi-perl libdbd-mysql-perl libparallel-forkmanager-perl \\"
	   echo "libnet-ping-external-perl libwww-perl libnet-ip-perl libsnmp-perl libdate-manip-perl \\"
	   echo "libdate-calc-perl libtime-modules-perl libmailtools-perl"
	fi
	echo
	echo "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
    elif [ "$LINUX_DIST" = "suse" ]
...
do
    ...
    then
        ...
        echo "*** ERROR - ro user ($RO_USER) was NOT created - Did you execut the commands above?"
        res=0
    else
    ...
    then
        ...
        echo "*** ERROR - rw user ($RW_USER) was NOT created - Did you execut the commands above?"
        res=0
    else
...
then
	...
	then
		...
		then
			...
			$WGET -w 2 -T 5 -t 6 http://www.gestioip.net/docu/${TE_FILE} >> $SETUP_LOG 2>&1
			if [ $? -ne 0 ]
			then
				echo "FAILED"
...
echo "+-------------------------------------------------------+"
echo
echo "GestioIP installation successfull" >> $SETUP_LOG
