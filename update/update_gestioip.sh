...
echo
echo "This script will update GestioIP to version $VERSION"
echo
...
echo $UPDATE_DIR | grep gestioip_2.2 | egrep "update$" >/dev/null 2>&1
if [ $? -ne 0 ]
then
    echo "Please run this script from the folder gestioip_2.2.x/update"
    echo 
    echo "Update aborted!" >> $UPDATE_LOG 2>&1
    echo "Update aborted!"
    echo
...
then
	echo "Actual Version $VERSION is already installed" >> $UPDATE_LOG
	echo "Actual Version $VERSION is already installed"
	echo ""
	echo "Update aborted!" >> $UPDATE_LOG 2>&1
	echo "Update aborted!"
	echo ""
...
tar -P -zcf ./$BACKUP_DATE.backup_update_gestioip.tar.gz $GESTIOIP_DOCUMENT_ROOT >> $UPDATE_LOG 2>/dev/null
echo "Storing BACKUP of old installation in $UPDATE_DIR/$BACKUP_DATE.backup_gestioip_update.tar.gz"
echo "Storing BACKUP of old installation in $UPDATE_DIR/$BACKUP_DATE.backup_gestioip_update.tar.gz" >> $UPDATE_LOG
echo
...
echo "++ GestioIP $VERSION requieres new Perl modules ++"
echo
...
then
    ...
    then
            ...
            then
                ...
                then
                   echo
                   echo "Executing yum install perl-Net-IP perl-Net-Ping-External perl-Parallel-ForkManager perl-DBI perl-DBD-mysql perl-Spreadsheet-ParseExcel net-snmp-perl perl-DateManip perl-Date-Calc perl-TimeDate perl-MailTools perl-SNMP-Info perl-Net-DNS"
                   echo
                   sudo yum install perl-Net-IP perl-Net-Ping-External perl-Parallel-ForkManager perl-DBI perl-DBD-mysql perl-Spreadsheet-ParseExcel net-snmp-perl perl-DateManip perl-Date-Calc perl-TimeDate perl-MailTools perl-SNMP-Info perl-Net-DNS

                else
                   echo
                   echo "Executing yum perl-Net-IP perl-Net-Ping-External perl-Parallel-ForkManager perl-DBI perl-DBD-mysql perl-DateManip net-snmp-perl perl-Date-Calc perl-TimeDate perl-MailTools perl-SNMP-Info perl-Net-DNS"
                   echo
                   sudo yum install perl-Net-IP perl-Net-Ping-External perl-Parallel-ForkManager perl-DBI perl-DBD-mysql perl-DateManip net-snmp-perl perl-Date-Calc perl-TimeDate perl-MailTools perl-SNMP-Info perl-Net-DNS
		...
		then
            ...
            then
                echo
                echo "Executing sudo yum install perl-Net-IP perl-DBI perl-DBD-mysql perl-DateManip net-snmp-perl perl-Date-Calc perl-TimeDate perl-MailTools"
                echo
                sudo yum install perl-Net-IP perl-DBI perl-DBD-mysql perl-DateManip net-snmp-perl perl-Date-Calc perl-TimeDate perl-MailTools
                if  [ "$INSTALL_MOD_EXCEL" != "yes" ]
                then
                        OLE_STORAGE_LIGHT_MISSING="0"
                        PARSE_EXCEL_MISSING="0"
            ...
            then
                echo
                echo "Executing sudo yum install perl-Net-IP perl-DBI perl-DBD-mysql perl-DateManip net-snmp-perl perl-Date-Calc perl-TimeDate perl-MailTools perl-Net-DNS"
                echo
                sudo yum install perl-Net-IP perl-DBI perl-DBD-mysql perl-DateManip net-snmp-perl perl-Date-Calc perl-TimeDate perl-MailTools perl-Net-DNS
                if  [ "$INSTALL_MOD_EXCEL" != "yes" ]
                then
                        OLE_STORAGE_LIGHT_MISSING="0"
                        PARSE_EXCEL_MISSING="0"
            ...
            then
                echo
                echo "sudo zypper install Perl-DBD-mysql perl-DBI Perl-Net-IP perl-libwww-perl perl-SNMP perl-MailTools perl-Time-modules perl-Date-Calc perl-Date-Manip perl-Net-DNS"
                echo
                sudo zypper install Perl-DBD-mysql perl-DBI Perl-Net-IP perl-libwww-perl perl-SNMP perl-MailTools perl-Time-modules perl-Date-Calc perl-Date-Manip perl-Net-DNS
                if  [ "$INSTALL_MOD_EXCEL" != "yes" ]
                then
                        OLE_STORAGE_LIGHT_MISSING="0"
                        PARSE_EXCEL_MISSING="0"
            ...
            then
                ...
                then
                        ...
                        echo
                        echo "Installation aborted - no Universe repository" >> $UPDATE_LOG
                        echo "Installation aborted"
                        echo
                        exit 1
                fi
                ...
                then
                   echo
                   echo "Executing apt-get install libdbi-perl libdbd-mysql-perl libparallel-forkmanager-perl libnet-ping-external-perl libwww-perl libnet-ip-perl libspreadsheet-parseexcel-perl libsnmp-perl libdate-manip-perl libdate-calc-perl libtime-modules-perl libmailtools-perl libnet-dns-perl libsnmp-info-perl"
                   echo
                   sudo apt-get install libdbi-perl libdbd-mysql-perl libparallel-forkmanager-perl libnet-ping-external-perl libwww-perl libnet-ip-perl libspreadsheet-parseexcel-perl libsnmp-perl libdate-manip-perl libdate-calc-perl libtime-modules-perl libmailtools-perl libnet-dns-perl libsnmp-info-perl
		   ...
		   SNMP_INFO_AUTO_INSTALL=1
                   ...
                   echo
                   echo "Executing  apt-get install libdbi-perl libdbd-mysql-perl libparallel-forkmanager-perl libnet-ping-external-perl libwww-perl libnet-ip-perl libsnmp-perl libdate-manip-perl libdate-calc-perl libtime-modules-perl libmailtools-perl libnet-dns-perl libsnmp-info-perl"
                   echo
                   sudo apt-get install libdbi-perl libdbd-mysql-perl libparallel-forkmanager-perl libnet-ping-external-perl libwww-perl libnet-ip-perl libsnmp-perl libdate-manip-perl libdate-calc-perl libtime-modules-perl libmailtools-perl libnet-dns-perl libsnmp-info-perl
		...
		then
			...
			if [ -z "$input" ] || [ "$input" = "y" ] || [ "$input" = "Y" ] || [ "$input" = "yes" ]
			then
				rm -r ./netdisco-mibs-0.7* >> $UPDATE_LOG 2>&1
				echo -n "Downloading Netdisco MIBs (this may take several minutes)... "
				$WGET -w 2 -T 5 -t 6 http://sourceforge.net/projects/netdisco/files/netdisco-mibs/0.7/netdisco-mibs-0.7.tar.gz >> $UPDATE_LOG 2>&1
				if [ $? -ne 0 ]
				then
					echo "FAILED"
					...
					echo
					echo "Please install Netdisco-MIBs v0.7 manually after installation has finished ***"
					echo "(Download netdisco-mibs from https://sourceforge.net/projects/netdisco/files/netdisco-mibs/)"
					echo "and copy the content of netdisco-mibs-0.7/ to /usr/share/gestioip/mibs/"
					echo

				else
					if [ -e "./netdisco-mibs-0.7.tar.gz" ]
					then
						echo "OK" >> $UPDATE_LOG
						echo "OK"

						tar -vzxf netdisco-mibs-0.7.tar.gz >> $UPDATE_LOG 2>&1
						mkdir -p /usr/share/gestioip/mibs  >> $UPDATE_LOG 2>&1
						if [ -w "/usr/share/gestioip/mibs" ]
						then
							cp -r ./netdisco-mibs-0.7/* /usr/share/gestioip/mibs/ >> $UPDATE_LOG 2>&1
                ...
                do
                        ...
                        then
                                ...
                                $WGET -w 2 -T 5 -t 6 http://search.cpan.org/~jmcnamara/ >> $UPDATE_LOG 2>&1
                        ...
                        then
                                ...
                                $WGET -w 2 -T 5 -t 6 http://search.cpan.org/~dlux/ >> $UPDATE_LOG 2>&1
                        ...
                        then
                                ...
                                $WGET -w 2 -T 5 -t 6 http://search.cpan.org/~chorny/ >> $UPDATE_LOG 2>&1
                        ...
                        then
                                ...
                                $WGET -w 2 -T 5 -t 6 http://search.cpan.org/~maxb/ >> $UPDATE_LOG 2>&1
                        ...
                        then
                                ...
                                $WGET -w 2 -T 5 -t 6 http://search.cpan.org/~olaf/ >> $UPDATE_LOG 2>&1
			else
				continue
                        fi
                        ...
                        $WGET -w 2 -T 5 -t 6 $URL_COMPL >> $UPDATE_LOG  2>&1
                        if [ $? -ne 0 ]
                        then
		...
		echo
			   ...
			   echo "SNMP-Info: http://search.cpan.org/~maxb/"
                    ...
                    then
                        ...
                        echo "perl-SNMP perl-MailTools perl-Time-modules perl-Date-Calc perl-Date-Manip perl-Net-DNS"
                        echo 
			...
			echo "SNMP::Info: http://search.cpan.org/~maxb/"
                        ...
                        echo "Installation aborted - Perl Modules missing" >> $UPDATE_LOG
                        echo "Installation aborted"
                        echo
                        exit 1
                else
    ...
    echo
    echo
    echo "Installation aborted! - Perl Modules missing 2" >> $UPDATE_LOG
    echo "Installation aborted!"
    echo
    exit 1
 fi
     ...
     echo "Please install missing Perl Modules manually and execute update_gestioip.sh again"
     echo
     echo "Installation aborted! - Perl Modules missing - UNKNOWN LINUX" >> $UPDATE_LOG
     echo "Installation aborted"
     exit 1
 fi
...
echo "Storing backup of actual database in $UPDATE_DIR/$BACKUP_DATE.backup_gestioip_mysql.sql"
echo "Storing backup of actual database in $UPDATE_DIR/$BACKUP_DATE.backup_gestioip_mysql.sql" >> $UPDATE_LOG 
echo "mysqldump -u $DATABASE_USER -pxxxxxxx -h $DATABASE_HOST -P $DATABASE_PORT $DATABASE_NAME > $UPDATE_DIR/$BACKUP_DATE.backup_gestioip_mysql.sql" >> $UPDATE_LOG
mysqldump -u $DATABASE_USER -p$DATABASE_PASSWORD -h $DATABASE_HOST -P $DATABASE_PORT $DATABASE_NAME > $UPDATE_DIR/$BACKUP_DATE.backup_gestioip_mysql.sql 2>> $UPDATE_LOG
...
echo "cp ../scripts/ip_update_gestioip.conf ${SCRIPT_CONF_DIR}/" >> $UPDATE_LOG 2>&1
...
then
	...
	then
		...
		then
			...
			$WGET -w 2 -T 5 -t 6 http://www.gestioip.net/docu/${TE_FILE} >> $UPDATE_LOG 2>&1
			if [ $? -ne 0 ]
			then
				echo "FAILED"
...
echo "|                                    |"
echo "|             Update to              |"
echo "|          GestioIP $VERSION            |"
echo "|      has finished successfully!    |"
echo "|                                    |" 
echo "+------------------------------------+"                             
echo ""
echo "Note:"
echo "Don't forget to update cronjobs for"
echo "automatic actualization. New scripts"
echo "are installed under ${SCRIPT_BASE_DIR}/bin"
echo ""
